...
<template>
  <div class="space-y-4">
    ...
    <div>
      ...
      <div v-else-if="!currentAccountId" class="farm-card rounded-2xl bg-white p-8 text-center text-gray-500 shadow-md dark:bg-gray-800">
        请选择账号后查看数据分析
      </div>

      <div v-else-if="list.length === 0" class="farm-card rounded-2xl bg-white p-8 text-center text-gray-500 shadow-md dark:bg-gray-800">
        暂无数据
      </div>
      ...
      <div v-else-if="activeTab === 'crops'" class="space-y-4">
        ...
        <div class="p-4 space-y-4">
          <div class="block sm:hidden space-y-4">
            <div v-for="(item, idx) in sortedList" :key="idx" class="cartoon-card border border-gray-200 rounded-2xl bg-white p-4 shadow-md dark:border-gray-700 dark:bg-gray-800">
              ...
              <div class="mt-3">
                <button
                  class="cartoon-btn w-full rounded-xl px-3 py-2 text-sm transition"
          ...
          <div class="farm-card hidden overflow-hidden border border-gray-200 rounded-2xl bg-white shadow-md sm:block dark:border-gray-700 dark:bg-gray-800">
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                  <tr v-for="(item, idx) in sortedList" :key="idx" class="group transition-colors hover:bg-gray-50 dark:hover:bg-gray-700/50">
                    ...
                    <td class="px-4 py-2 text-center">
                      <button
                        class="cartoon-btn rounded-xl px-3 py-1.5 text-xs transition"
                        :class="blacklist.includes(item.seedId)
                          ? 'bg-red-100 text-red-700 hover:bg-red-200 dark:bg-red-900/30 dark:text-red-400 dark:hover:bg-red-900/50'
                          : 'bg-gray-100 text-gray-600 hover:bg-gray-200 dark:bg-gray-700 dark:text-gray-300 dark:hover:bg-gray-600'"
                        @click="handleToggleBlacklist(item)"
        ...
        <div class="p-4">
          ...
          <div v-else class="space-y-3">
            <div
              ...
              :key="seedId"
              class="cartoon-card flex items-center justify-between rounded-xl bg-gray-50 px-4 py-3 dark:bg-gray-700/50"
            >
              <div class="flex items-center gap-3">
                <div class="relative h-10 w-10 flex shrink-0 items-center justify-center overflow-hidden border border-gray-200 rounded-lg bg-gray-100 dark:border-gray-600 dark:bg-gray-700">
                  <img
